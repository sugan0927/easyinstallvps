...
# ============================================
# EasyInstall WordPress + Static HTML Maximum Performance Installation Script v6.5
...
# PHP: 8.5 / 8.4 / 8.3 / 8.2 (co-installed, highest available is primary)
# Site types: WordPress (default) or plain HTML/static (--html)
# Fully tested on: Debian 12 (bookworm), Debian 13 (trixie),
#                  Ubuntu 22.04 LTS, Ubuntu 24.04 LTS, Ubuntu 26.04 LTS
# Best-effort (warns, doesn't block): Debian 11, Ubuntu 20.04, other
#                  Ubuntu point releases
# Not supported: any non-Debian/Ubuntu distro (RHEL/CentOS/Fedora/Alma/
#                  Rocky, Arch, openSUSE, ...) — see README for why
#
# ARCHITECTURE:
...
#                            WordPress/HTML setup, Nginx/PHP/MySQL/Redis config,
...
perform_rollback() {
    ...
    for service in nginx php8.5-fpm php8.4-fpm php8.3-fpm php8.2-fpm mariadb redis-server; do
...
check_os_compatibility() {
    ...
    [[ ! "$OS_ID" =~ ^(debian|ubuntu)$ ]] && {
        ...
        log "ERROR" "EasyInstall targets Debian/Ubuntu (apt-based). Other distros (RHEL/CentOS/Fedora/Alma/Rocky, Arch, openSUSE, etc.) are not supported — the repo setup, package names and paths throughout this script are Debian/Ubuntu-specific."
        return 1
    }

    # Debian: 12 (bookworm) and 13 (trixie) are fully tested. 11 (bullseye)
    # still works (older PHP/MariaDB repo branches exist for it) but is past
    # its ideal window. Anything below 11 is refused outright.
    if [ "$OS_ID" = "debian" ]; then
        ...
        if [ -z "$major" ] || [ "$major" -lt 11 ]; then
            ...
            log "WARNING" "Debian 11 (bullseye) detected — supported, but 12/13 are the tested targets. Some upstream repos may drop bullseye over time."
        elif [ "$major" -ge 14 ]; then
            log "WARNING" "Debian $OS_VERSION is newer than the tested range (12–13) — proceeding, but watch for repo/package-name changes upstream."
        fi
    fi

    # Ubuntu: 22.04 and 24.04 LTS are fully tested; 26.04 LTS is supported
    # (Sury/Ondrej already publish PHP packages for it). Non-LTS interim
    # releases and 20.04 (EOL for standard support) get a warning, not a
    # hard stop, since the underlying apt/systemd tooling still works.
    if [ "$OS_ID" = "ubuntu" ]; then
        case "$OS_VERSION" in
            22.04|24.04|26.04) : ;;
            20.04)
                log "WARNING" "Ubuntu 20.04 detected — standard support has ended upstream; PHP/MariaDB repos for it may disappear. Upgrading to 22.04+ is recommended." ;;
            *)
                log "WARNING" "Ubuntu $OS_VERSION is not one of the tested releases (22.04/24.04/26.04 LTS) — continuing, but you may hit repo or package-name mismatches." ;;
        esac
    fi
...
install_php_packages() {
    ...
    # Try PHP 8.5 → 8.4 → 8.3 → 8.2 (highest available becomes the primary/
    # active version; older ones are still installed as co-existing fallbacks
    # so existing sites pinned to an older version keep working).
    ...
    for ver in 8.5 8.4 8.3 8.2; do
        ...
        pkgs="$pkgs php${ver}-xml php${ver}-zip php${ver}-soap php${ver}-intl"
        pkgs="$pkgs php${ver}-bcmath php${ver}-imagick php${ver}-redis"
        pkgs="$pkgs php${ver}-readline php${ver}-apcu php${ver}-memcached php${ver}-igbinary"

        # FIX: `apt-get install` is all-or-nothing — one nonexistent package
        # name fails the ENTIRE command, silently skipping that whole PHP
        # version. Some optional extensions aren't packaged the same way on
        # every branch (opcache is bundled into PHP core as of 8.5 rather
        # than shipped as its own php8.5-opcache package; xmlrpc has been a
        # PECL-only extension since PHP 8.0 and Sury doesn't always package
        # it for the newest branch yet). Probe availability first and only
        # add these to the install list when the package genuinely exists,
        # instead of hardcoding them and breaking the whole version.
        for optional_ext in opcache xmlrpc; do
            if apt-cache show "php${ver}-${optional_ext}" >/dev/null 2>&1; then
                pkgs="$pkgs php${ver}-${optional_ext}"
            fi
        done

        if run_cmd_retry 3 5 "apt-get install -y $pkgs"; then
            # FIX: only the FIRST (highest) successful version becomes the
            # primary — previously this was overwritten on every iteration,
            # so it silently ended up as the LOWEST version installed (8.2)
            # instead of the intended highest one.
            [ -z "$PHP_INSTALLED_VERSION" ] && PHP_INSTALLED_VERSION=$ver
...
# ============================================
# SECTION 18B — VERIFY / REPAIR OPCACHE (BASH)
# Additive-only safeguard: does not modify
# install_php_packages(), run_python_config(),
# or enable_start_php() in any way.
#
# ROOT CAUSE: the Python "php_config" stage
# regenerates PHP conf.d files using an atomic
# write (write temp file -> rename). On this
# server, that rename can replace the symlink
# that `phpenmod` created for
# /etc/php/<ver>/fpm/conf.d/10-opcache.ini,
# which points at mods-available/opcache.ini
# (the file with `zend_extension=opcache.so`).
# If that symlink is lost, PHP-FPM keeps running
# (enable_start_php only calls `systemctl start`,
# which is a no-op if the service is already
# active from the package install) without ever
# reloading OPcache — so WordPress Site Health
# reports "OPcache is not enabled" even though the
# php*-opcache package is installed.
#
# This function checks each installed PHP version,
# restores the symlink if it's missing or broken,
# and only then issues a full `restart` (not just
# `start`) so the change actually takes effect.
# ============================================
ensure_opcache_enabled() {
    log "STEP" "Verifying OPcache is enabled for all installed PHP versions"
    local fixed_any=0

    for version in 8.5 8.4 8.3 8.2; do
        [ -d "/etc/php/$version" ] || continue

        local mods_ini="/etc/php/$version/mods-available/opcache.ini"
        local fpm_ini="/etc/php/$version/fpm/conf.d/10-opcache.ini"
        local cli_ini="/etc/php/$version/cli/conf.d/10-opcache.ini"

        # phpenmod is idempotent and safe to re-run; this recreates the
        # symlink if a later config-write step clobbered it.
        if command -v phpenmod &>/dev/null; then
            phpenmod -v "$version" opcache 2>/dev/null || true
        fi

        # If mods-available/opcache.ini itself is missing (rare), recreate
        # a minimal, standard one so phpenmod has a valid target.
        if [ ! -f "$mods_ini" ]; then
            log "WARNING" "PHP $version: opcache.ini missing from mods-available — recreating"
            cat > "$mods_ini" <<'OPCACHEINI'
; priority=10
...
OPCACHEINI
        ...
        # Confirm the FPM conf.d entry exists AND actually loads the
        # extension. If write_file's atomic rename replaced the symlink
        # with a plain file missing zend_extension, or removed it outright,
        # repair it directly.
        if [ ! -e "$fpm_ini" ]; then
            log "WARNING" "PHP $version: FPM opcache symlink missing at $fpm_ini — recreating"
            ln -sf "$mods_ini" "$fpm_ini"
            fixed_any=1
        elif ! grep -q "^zend_extension=opcache" "$fpm_ini" 2>/dev/null; then
            log "WARNING" "PHP $version: FPM opcache config present but zend_extension line missing — repairing"
            ln -sf "$mods_ini" "$fpm_ini"
            fixed_any=1
        fi

        # Keep CLI in sync too (affects wp-cli performance, not Site Health,
        # but should match).
        [ -e "$cli_ini" ] || ln -sf "$mods_ini" "$cli_ini" 2>/dev/null || true
    done

    # Only force a full restart if we actually repaired something —
    # `start` (used by enable_start_php) is a no-op on an already-running
    # service, so a real `restart` is required to load the fix.
    if [ "$fixed_any" -eq 1 ]; then
        for version in 8.5 8.4 8.3 8.2; do
            [ -d "/etc/php/$version" ] || continue
            systemctl restart "php$version-fpm" 2>/dev/null && \
                log "INFO" "PHP $version-FPM restarted to load repaired OPcache config" || \
                log "WARNING" "Could not restart php$version-fpm — restart manually to apply OPcache fix"
        done
    fi

    # Final verification against the live module list.
    for version in 8.5 8.4 8.3 8.2; do
        [ -d "/etc/php/$version" ] || continue
        if php$version -m 2>/dev/null | grep -qi "^Zend OPcache$"; then
            log "SUCCESS" "OPcache confirmed enabled for PHP $version"
        else
            log "ERROR" "OPcache still not enabled for PHP $version — check $fpm_ini manually"
        fi
    done
}
...
phase_preflight() {
    clear
    echo -e "${GREEN}========================================${NC}"
    echo -e "${GREEN}🚀 EasyInstall WordPress + HTML Performance v6.5 (HYBRID EDITION)${NC}"
...
# ── PHASE Q0: Python — security hardening + malware scanner ────────
# FIX (v6.5): these two stages have existed in easyinstall_config.py since
# v7.0 (security headers, WP hardening snippet, ClamAV-based scanner) but
# were never actually called from anywhere in this script or exposed via
# any CLI command — they were dead code that only ran if someone happened
# to invoke `python3 easyinstall_config.py --stage stage_security_hardening`
# by hand. Wiring them into the default install so the security posture
# this package advertises actually gets applied.
phase_security_hardening() {
    run_python_config "stage_security_hardening" || log "WARNING" "security_hardening stage failed — continuing without extra hardening snippets"
    run_python_config "stage_malware_scanner"    || log "WARNING" "malware_scanner stage failed — continuing without ClamAV scanner"
    update_status "SECURITY" "Security hardening + malware scanner configured"
}
...
phase_monitoring_scripts() {
    ...
    for _v in 8.5 8.4 8.3 8.2; do test_php_fpm "$_v" 2>/dev/null || true; done
...
phase_finalize() {
    ...
    echo "   • PHP      : ${PHP_INSTALLED_VERSION:-n/a} (primary)"
    echo ""
    echo -e "${YELLOW}📋 Next Steps:${NC}"
    echo "   1.  source ~/.bashrc"
    echo "   2.  easyinstall help"
    echo "   3.  easyinstall create mysite.com                 # WordPress site"
    echo "   3b. [NEW] easyinstall create mysite.com --html    # Plain HTML/static site"
