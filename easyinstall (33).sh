...
phase_monitoring_scripts() {
    # None of these affect whether WordPress itself works — they're
    # operator-facing convenience tooling, so failures here are logged and
    # skipped rather than aborting an otherwise-successful install.
    run_python_config "create_redis_monitor"   || log "WARNING" "create_redis_monitor failed — skipping"
    run_python_config "create_commands"        || log "WARNING" "create_commands failed — the 'easyinstall' CLI may be unavailable"
