...
function db_connect(): ?PDO {
    ...
    // Try unix_socket first — MariaDB on Debian/Ubuntu uses unix_socket plugin
    // for root by default, so no password is needed when connecting via socket.
    $sockets = [
    ...
    foreach ($sockets as $sock) {
        ...
        try {
            return new PDO("mysql:unix_socket={$sock};charset=utf8mb4", 'root', '', $opts);
        } catch (PDOException $e) {
            // socket exists but auth still failed — try next
        }
    }

    // Fallback: TCP with empty password (works when native password auth is set)
    try {
        return new PDO('mysql:host=127.0.0.1;port=3306;charset=utf8mb4', 'root', '', $opts);
    } catch (PDOException $e) {
        err("DB connect failed: " . $e->getMessage());
        err("Fix: mysql -e \"ALTER USER 'root'@'localhost' IDENTIFIED VIA mysql_native_password USING ''; FLUSH PRIVILEGES;\"");
        return null;
    }
}
