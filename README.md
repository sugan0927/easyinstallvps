# EasyInstall v7.0 - Documentation Enhancement & Feature Roadmap
 ## Install on your VPS — Debian 11 & 12 fully supported, also supports Ubuntu 20.04 / 22.04 / 24.04
 # One-Click Install Command
 ## wget -qO- ea.ez-ins.site | bash
## 📚 Current Documentation Status Analysis

> **✅ Update note:** This document has now been reconciled against the live `easyinstall_config.py` dispatcher. Every command shown below (in the README draft, INSTALL.md, and COMMANDS.md sections) was checked against the actual `case` statement in the script — the command reference is now complete (47/47 real commands documented) and no longer references commands that don't exist. Anything still marked "Phase 2" or "Roadmap" further down is a **planned, not-yet-built** feature — those are unchanged and still aspirational.

### Existing Documentation Assessment

| **Document Type** | **Current State** | **Quality** | **Gap Analysis** |
|-------------------|-------------------|-------------|------------------|
| **README/Intro** | ✅ Rewritten below | ⭐⭐⭐⭐ | Structure, badges, and a verified install command in place |
| **Installation Guide** | ✅ Standalone draft below | ⭐⭐⭐⭐ | Covers install, verification, upgrade, uninstall |
| **API/Commands** | ✅ Full 47-command reference below | ⭐⭐⭐⭐⭐ | Cross-checked against the live dispatcher; no more phantom commands |
| **Architecture** | ✅ Draft below | ⭐⭐⭐⭐ | Covers services, config hierarchy, tuning pipeline, data flow |
| **Troubleshooting** | ⚠️ Partial | ⭐⭐ | Covers common install/perf issues; still light on edge cases |
| **Performance Tuning** | ✅ Manual + auto-tune guide below | ⭐⭐⭐⭐ | Includes formulas and manual override paths |
| **Security Hardening** | ⚠️ In code comments + scattered mentions | ⭐⭐ | Still no standalone SECURITY.md |
| **Migration Guide** | ❌ Missing | - | No migration/comparison doc from competitors as a standalone file |
...
> Replace `https://raw.githubusercontent.com/yourrepo/easyinstall/...` style links throughout this document with your actual public repo URL once one exists — `yourrepo` is a placeholder, not a real path. The one-click command above (`wget -qO- ea.ez-ins.site | bash`) is the real, working install method.
...
easyinstall site-info domain.com                     # Full details for one site
easyinstall ssl domain.com                           # Enable SSL
easyinstall ssl-renew                                # Renew all SSL certificates
easyinstall clone src.com dst.com                    # Clone site
easyinstall update-site domain.com                   # Update core/plugins/themes
easyinstall php-switch domain 8.4                    # Switch PHP version (no downtime)
easyinstall remote-install domain.com --ssl          # Install on a remote VPS via SSH
```

### Performance Commands
```bash
easyinstall advanced-tune         # Run all 10 auto-tuning phases
easyinstall perf-dashboard        # Live performance dashboard
easyinstall perf                  # Quick performance snapshot
easyinstall warm-cache            # Smart cache warmer
easyinstall db-optimize           # Database optimization report
easyinstall wp-speed              # WordPress-level speed tweaks
easyinstall optimize              # Manual optimization run (clear caches, optimize tables)
easyinstall install-governor      # Install the dynamic resource governor
easyinstall emergency-check       # Run disaster-recovery diagnostics
easyinstall autotune-rollback     # Revert the last auto-tune pass
easyinstall clean                 # Clean temp files / old logs / caches
```

### AI Commands
```bash
easyinstall ai-setup              # Configure AI (Ollama/OpenAI/Groq/Gemini)
easyinstall ai-install-ollama     # Install local Ollama for on-server AI
...
easyinstall redis-fix-ports       # Repair a corrupted/conflicting port map
...
easyinstall ws-disable domain      # Disable WebSocket proxy
easyinstall ws-status              # Show WebSocket status for all sites
easyinstall ws-test domain         # Test the WebSocket upgrade handshake
...
easyinstall version                # Show installed EasyInstall version
easyinstall backup [daily|weekly]  # Create a full-server backup
easyinstall backup-site domain     # Backup specific site (files + database)
easyinstall logs                   # View recent logs
easyinstall nginx-extras           # Apply Brotli / Cloudflare / SSL-hardening extras
```

> **Not implemented (do not rely on these):** there is currently no standalone `restore` / `restore-site` command — `backup` and `backup-site` only *create* backups; restoring is a manual `tar`/`mysql` step today. `backup-status`, `security-status`, `redis-info`, `redis-monitor`, `redis-cluster-setup`, and any `cloudflare-*` subcommand (`cloudflare-setup`, `cloudflare-status`, etc.) are **not** real commands in the current script, even if you see them referenced in older drafts or blog content.
...
| `/etc/easyinstall/ai.conf` | AI provider config (Ollama/OpenAI/Groq/Gemini) |
| `/usr/local/lib/easyinstall-ai.sh` | AI module functions |
| `/usr/local/lib/easyinstall-autotune.sh` | Auto-tune engine + rollback library |
...
### Method 2: Manual Install (once you have a public repo/release URL)

```bash
# Download the scripts (replace with your actual hosting URL)
wget https://your-domain-or-repo/easyinstall.sh
wget https://your-domain-or-repo/easyinstall_config.py
...
```bash
git clone https://github.com/yourrepo/easyinstall.git   # replace with your real repo URL
cd easyinstall
sudo ./easyinstall.sh
```
...
# Re-run the one-click installer (it detects an existing install and upgrades in place)
wget -qO- ea.ez-ins.site | bash

# Always back up every site before a version upgrade
easyinstall backup-site domain.com   # repeat per site, or script over `easyinstall list`
```
...
Request and install a Let's Encrypt SSL certificate for a domain that doesn't already have one.

```bash
easyinstall ssl mysite.com
```

---

### `easyinstall ssl-renew`
Renew SSL certificates across all sites (also runs automatically via cron).
...
Show full detail for a single site — Redis port, PHP version, SSL status, disk usage.

```bash
easyinstall site-info mysite.com
```
...
### `easyinstall wp-speed`
Apply WordPress-level speed tweaks (heartbeat throttling, cron handling) on top of server-level tuning.
...
### `easyinstall install-governor`
Install the dynamic resource governor, which watches load and reacts to sustained resource pressure.
...
### `easyinstall emergency-check`
Run a broader disaster-recovery diagnostic pass and apply safe, conservative recovery steps if the server is under serious stress.
...
### `easyinstall autotune-rollback`
Revert configuration changes made by the last `advanced-tune` pass back to the previous known-good state.
...
### `easyinstall perf`
Quick performance snapshot (lighter-weight than the full `perf-dashboard`).
...
### `easyinstall clean`
Clean temporary files, rotate old logs, and clear stale cache artifacts.
...
### `easyinstall ai-install-ollama`
Install Ollama locally so AI commands can run against a local model with no data leaving the server.
...
### `easyinstall redis-fix-ports`
Repair the Redis port map if it becomes corrupted or a manual edit causes a port conflict between sites.
...
Disable WebSocket proxying for a site that no longer needs it.

```bash
easyinstall ws-disable mysite.com
```
...
Run a connection test to confirm the WebSocket upgrade handshake actually completes end-to-end.

```bash
easyinstall ws-test mysite.com
```
...
### `easyinstall version`
Show the installed EasyInstall version.
...
1. ✅ README rewritten with structure, badges, and a corrected one-click install command
2. ✅ Standalone installation guide (INSTALL.md draft) included above
3. ✅ Command reference now covers all 47 real commands, cross-checked against `easyinstall_config.py` — previously undocumented commands (`redis-fix-ports`, `ws-disable`, `ws-test`, `ssl-renew`, `wp-speed`, `site-info`, `install-governor`, `emergency-check`, `autotune-rollback`, `perf`, `clean`, `version`, `ai-install-ollama`) have been added, and a warning box now calls out commands that **don't** exist (`restore`, `restore-site`, `backup-status`, `cloudflare-*`, `redis-info`, `redis-monitor`, `redis-cluster-setup`, `security-status`)
4. ✅ Architecture documentation (ARCHITECTURE.md draft) included above
5. ⚠️ Troubleshooting guide exists but is still fairly minimal — worth expanding with more real-world error cases over time
6. ✅ Performance tuning guide (PERFORMANCE.md draft) included above
7. ❌ Still no dedicated migration guide from competitors (EasyEngine/WordOps/etc.) as a standalone doc
8. ❌ Still no formal API documentation (relevant only if a REST/HTTP API is ever exposed beyond the CLI)
9. ❌ Still missing contribution guidelines (CONTRIBUTING.md)
